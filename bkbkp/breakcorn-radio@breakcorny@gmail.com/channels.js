...
// import * as Data from "./data.js"; // Removed to avoid circular dependency
...
// Get channels without favorite status (to avoid circular dependency)
export function getChannelsRaw() {
  ...
  return allChannels.map(
    (ch) => new Channel(ch.name, ch.link, ch.pic, ch.num, false),
  );
}

// Get channels with favorite status (requires Data module)
export function getChannels() {
  // This function will be called from extension.js where Data is available
  const isDecember = new Date().getMonth() === 11;
  const allChannels = isDecember ? [...channels, ...holidayChannels] : channels;
  // Import Data dynamically to avoid circular dependency
  return allChannels.map(
    (ch) => new Channel(ch.name, ch.link, ch.pic, ch.num, false), // Will be updated by caller
  );
}

export function getFavChannels(favsList) {
  // Accept favorites list as parameter to avoid circular dependency
  return getChannelsRaw()
    .filter((ch) => favsList && favsList.includes(ch.num))
    .map((ch) => new Channel(ch.name, ch.link, ch.pic, ch.num, true));
}

export function getChannel(index) {
  let items = getChannelsRaw();
  let item = items[index] ?? items[0];
  return new Channel(
    item.name,
    item.link,
    item.pic,
    item.num,
    false, // Favorite status will be set by caller if needed
  );
}
